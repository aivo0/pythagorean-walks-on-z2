...
def _fits_signed_i64(value: int) -> bool:
    return SIGNED_I64_MIN <= value < SIGNED_I64_MAX


def _all_signed_i64(values: Iterable[int]) -> bool:
    return all(_fits_signed_i64(value) for value in values)


def _norm_sq_fits_i128(dx: int, dy: int) -> bool:
    return dx * dx + dy * dy <= SIGNED_I128_MAX
...
def edge_delta(dx: int, dy: int) -> bool:
    if _fast is not None and _all_signed_i64((dx, dy)) and _norm_sq_fits_i128(dx, dy):
...
def is_two_step_certificate(target: Point, midpoint: Point) -> bool:
    ...
    if (
        _fast is not None
        and _all_signed_i64((*target, *midpoint))
        and _norm_sq_fits_i128(x, y)
        and _norm_sq_fits_i128(*second)
    ):
...
LatticePairStripCrtData = tuple[int, int, int, int, int, int | None, bool, bool]
...
def linear_delta_direction_certificate(
    ...
    if _fast is not None and _all_signed_i64((*target, *direction, *delta_coefficients)):
        try:
            midpoint = _fast.linear_delta_direction_midpoint(
                target,
                direction,
                delta_coefficients,
            )
...
def scale_signed_swap_certificate(
    certificate: Certificate,
    factor: int,
    x_sign: int,
    y_sign: int,
    ...
    """Scale a certificate and then apply a sign/swap graph automorphism."""

    if _fast is not None and all(
        -(1 << 63) <= coordinate < (1 << 63)
        for coordinate in (*certificate.target, *certificate.midpoint, factor)
    ):
        target, midpoint = _fast.scale_signed_swap_certificate_data(
            certificate.target,
            certificate.midpoint,
            factor,
            x_sign,
    ...
    scaled = scale_certificate(certificate, factor)
    return Certificate(
        target=signed_swap_point(scaled.target, x_sign, y_sign, swap),
        midpoint=signed_swap_point(scaled.midpoint, x_sign, y_sign, swap),
    )
...
def gaussian_root_shape(root: Point) -> Point:
    """Canonical unsigned shape of a nonzero Gaussian root."""

    root_real, root_imaginary = root
    if root_real == 0 or root_imaginary == 0:
        raise ValueError("root coordinates must be nonzero")
    first = abs(root_real)
    second = abs(root_imaginary)
...
@cache
def primitive_pythagorean_root_directions(
    max_root_coordinate: int,
) -> tuple[RootDirection, ...]:
    """Primitive signed Pythagorean directions generated by bounded roots.

    Each row is ``(u, v, c, alpha, unit)`` with
    ``(u, v) = unit*alpha^2`` and ``N(alpha)=c``.  This is the Gaussian-root
    form of ``primitive_pythagorean_directions``.
    """
    ...
    units: tuple[Point, ...] = ((1, 0), (-1, 0), (0, 1), (0, -1))
    seen: set[tuple[int, int, int]] = set()
    for root_real in range(-max_root_coordinate, max_root_coordinate + 1):
        for root_imaginary in range(-max_root_coordinate, max_root_coordinate + 1):
            if root_real == 0 or root_imaginary == 0:
                continue
            if gcd(abs(root_real), abs(root_imaginary)) != 1:
                continue
            if (root_real - root_imaginary) % 2 == 0:
                continue

            root = (root_real, root_imaginary)
            root_square = gaussian_multiply(root, root)
            hypotenuse = root_real * root_real + root_imaginary * root_imaginary
            for unit in units:
                direction = gaussian_multiply(unit, root_square)
                if not edge_delta(*direction):
                    raise AssertionError("bounded Gaussian root produced an illegal edge")
                seen.add((direction[0], direction[1], hypotenuse))

    rows: list[RootDirection] = []
    for u, v, hypotenuse in sorted(
        seen,
        key=lambda row: (row[2], abs(row[0]) + abs(row[1]), row[0], row[1]),
    ):
        root, unit = primitive_pythagorean_direction_gaussian_root((u, v))
        if root[0] * root[0] + root[1] * root[1] != hypotenuse:
            raise AssertionError("root direction row lost its hypotenuse")
...
@cache
def primitive_pythagorean_root_shape_directions(
    root_shapes: tuple[Point, ...],
) -> tuple[RootDirection, ...]:
    """Primitive signed Pythagorean directions generated by root shapes."""

    units: tuple[Point, ...] = ((1, 0), (-1, 0), (0, 1), (0, -1))
    seen: set[tuple[int, int, int]] = set()
    canonical_shapes: set[Point] = set()
    for root_shape in root_shapes:
        shape_real, shape_imaginary = gaussian_root_shape(root_shape)
        canonical_shapes.add((shape_real, shape_imaginary))
        if gcd(shape_real, shape_imaginary) != 1:
            raise ValueError("root shape must be primitive")
        if (shape_real - shape_imaginary) % 2 == 0:
            raise ValueError("root shape coordinates must have opposite parity")

        for base in ((shape_real, shape_imaginary), (shape_imaginary, shape_real)):
            for real_sign in (-1, 1):
                for imaginary_sign in (-1, 1):
                    root = (real_sign * base[0], imaginary_sign * base[1])
                    root_square = gaussian_multiply(root, root)
                    hypotenuse = shape_real * shape_real + shape_imaginary * shape_imaginary
                    for unit in units:
                        direction = gaussian_multiply(unit, root_square)
                        if not edge_delta(*direction):
                            raise AssertionError(
                                "Gaussian root shape produced an illegal edge"
                            )
                        seen.add((direction[0], direction[1], hypotenuse))
    ...
    for u, v, hypotenuse in sorted(
        ...
        root, unit = primitive_pythagorean_direction_gaussian_root((u, v))
        if gaussian_root_shape(root) not in canonical_shapes:
            raise AssertionError("root shape direction row lost its shape")
        rows.append((u, v, hypotenuse, root, unit))
    return tuple(rows)
...
def lattice_coefficients(
    ...
    if _fast is not None and all(
        -(1 << 63) <= coordinate < (1 << 63)
        for coordinate in (*target, *first_direction, *second_direction)
    ):
        coefficients = _fast.lattice_coefficients(
            target,
            first_direction,
            second_direction,
        )
...
def lattice_coefficient_cramer_data(
    target: Point,
    first_direction: Point,
    second_direction: Point,
) -> LatticeCoefficientCramerData:
    """Return Cramer determinant data for a two-direction lattice basis."""

    if (
        _fast is not None
        and hasattr(_fast, "lattice_coefficient_cramer_data")
        and _all_signed_i64(
            (*target, *first_direction, *second_direction)
        )
    ):
        return tuple(
            _fast.lattice_coefficient_cramer_data(
                target,
                first_direction,
                second_direction,
            )
    ...
    ux, uy = first_direction
    vx, vy = second_direction
    determinant_value = ux * vy - uy * vx
    if determinant_value == 0:
        raise ValueError("lattice directions must be independent")

    first_numerator = gx * vy - gy * vx
    second_numerator = ux * gy - uy * gx
    first_divisible = first_numerator % determinant_value == 0
    second_divisible = second_numerator % determinant_value == 0
    return (
        determinant_value,
        first_numerator,
        second_numerator,
        first_divisible,
        second_divisible,
        first_numerator // determinant_value if first_divisible else None,
        second_numerator // determinant_value if second_divisible else None,
    )
...
def lattice_two_step_certificate(
    ...
    if _fast is not None and all(
        ...
        try:
            midpoint = _fast.lattice_certificate_midpoint(
                target,
                first_direction,
                second_direction,
            )
...
@dataclass(frozen=True)
class ParallelDirectionConjugateIdealWitness:
    target: Point
    direction: Point
    squareclass: int
    split_factor: int
    ...
    @property
    def determinant_leg(self) -> int:
        return determinant(self.direction, self.target)
    ...
    def second_step(self) -> Point:
        ...
        if second_step is None:
            raise AssertionError("conjugate-ideal witness has no legal second step")
        return second_step

    @property
    def gaussian_quadratic_left(self) -> Point:
        unit = self.unit
        inverse_unit = (unit[0], -unit[1])
        doubled_target = (2 * self.target[0], 2 * self.target[1])
        return gaussian_multiply(inverse_unit, doubled_target)

    @property
    def gaussian_quadratic_right(self) -> Point:
        root_square = gaussian_multiply(self.root, self.root)
        beta_square = gaussian_multiply(self.beta, self.beta)
        return (
            2 * self.first_coefficient * root_square[0]
            + self.squareclass * beta_square[0],
            2 * self.first_coefficient * root_square[1]
            + self.squareclass * beta_square[1],
        )

    def valid(self) -> bool:
        if self.determinant_leg != (
            self.squareclass
            * self.split_factor
        ...
        if parallel_direction_squareclass_beta_split_root(
            self.direction,
            self.beta,
        ) != (self.split_factor, self.signed_paired_split_factor):
            return False
        if self.target != (
            self.midpoint[0] + self.second_step[0],
            self.midpoint[1] + self.second_step[1],
        ):
            return False
...
@dataclass(frozen=True)
class ParallelDirectionConjugateIdealRootCoverCensus:
    max_coordinate: int
    max_root_coordinate: int
    target_count: int
    structural_miss_count: int
    uncovered_targets: tuple[Point, ...]
    root_counts: tuple[tuple[Point, int], ...]
...
@dataclass(frozen=True)
class ParallelDirectionConjugateIdealRootShapeCoverCensus:
    max_coordinate: int
    root_shapes: tuple[Point, ...]
    target_count: int
    structural_miss_count: int
...
def parallel_direction_factor_witness_row_data(
    target: Point,
    direction: Point,
    factor: int,
) -> tuple[int, int, int, int, int, int] | None:
    """Return the factor-pair row data used by the fixed-direction witness."""

    if _fast is not None and all(
        -(1 << 63) <= coordinate < (1 << 63)
        for coordinate in (*target, *direction, factor)
    ):
        data = _fast.parallel_direction_factor_witness_row_data(
            target,
            direction,
            factor,
        )
    ...
    if witness is None:
        return None
    paired_factor = witness.determinant_leg * witness.determinant_leg // factor
    return (
        witness.determinant_leg,
        paired_factor,
        witness.other_leg,
        witness.scaled_hypotenuse,
        witness.second_length,
...
def parallel_direction_squareclass_beta_quadratic_coefficient(
    target: Point,
    direction: Point,
    squareclass: int,
    beta: Point,
) -> int | None:
    """First-step coefficient from ``2*unit^-1*T = 2r*alpha^2 + q*beta^2``."""
    ...
    inverse_unit = (unit[0], -unit[1])
    rotated_target = gaussian_multiply(
        inverse_unit,
        (2 * target[0], 2 * target[1]),
    )
    beta_square = gaussian_multiply(beta, beta)
    difference = (
        rotated_target[0] - squareclass * beta_square[0],
        rotated_target[1] - squareclass * beta_square[1],
    )
    quotient = gaussian_quotient_if_integer(difference, gaussian_multiply(root, root))
    if quotient is None or quotient[1] != 0 or quotient[0] % 2 != 0:
        return None
    return quotient[0] // 2


def parallel_direction_squareclass_beta_quadratic_certificate(
    target: Point,
    direction: Point,
    squareclass: int,
    beta: Point,
) -> Certificate | None:
    """Target-facing beta certificate from the rotated quadratic identity."""

    first_coefficient = parallel_direction_squareclass_beta_quadratic_coefficient(
        target,
        direction,
        squareclass,
        beta,
...
def parallel_direction_squareclass_conjugate_ideal_certificate(
    ...
    witness = parallel_direction_squareclass_conjugate_ideal_witness(
        target,
        direction,
        squareclass,
    )
...
def parallel_direction_squareclass_conjugate_ideal_witness(
    target: Point,
    direction: Point,
    squareclass: int,
) -> ParallelDirectionConjugateIdealWitness | None:
    """Witness from one squareclass in the conjugate-root ideal inverse."""
...
def parallel_direction_conjugate_ideal_split_roots(
    ...
    determinant_leg = determinant(direction, target)
    return parallel_direction_conjugate_ideal_determinant_roots(
        direction,
        determinant_leg,
    )


def parallel_direction_conjugate_ideal_determinant_roots(
    direction: Point,
    determinant_leg: int,
) -> tuple[tuple[int, int, int, Point], ...]:
    """All legal ``(q, a, b, beta)`` ideal rows for one determinant level."""
...
def parallel_direction_conjugate_ideal_certificate(
    ...
    witness = parallel_direction_conjugate_ideal_witness(target, direction)
    if witness is None:
        return None
    return witness.certificate


def parallel_direction_conjugate_ideal_witness(
    target: Point,
    direction: Point,
) -> ParallelDirectionConjugateIdealWitness | None:
    """Exact fixed-direction split witness over squarefree determinant divisors."""
...
def parallel_direction_conjugate_ideal_cover_certificate(
    ...
    for u, v, _hypotenuse, _parameter_a, _parameter_b in primitive_pythagorean_directions(
        ...
        witness = parallel_direction_conjugate_ideal_witness(target, (u, v))
        if witness is not None:
            return witness.certificate
    return None
...
@cache
def parallel_direction_conjugate_ideal_root_cover_certificate(
    target: Point,
    max_root_coordinate: int,
) -> Certificate | None:
    """Root-bounded exact split cover using conjugate-root divisor roots."""

    witness = parallel_direction_conjugate_ideal_root_cover_witness(
        target,
        max_root_coordinate,
    )
    if witness is None:
...
@cache
def parallel_direction_conjugate_ideal_root_shape_cover_certificate(
    target: Point,
    root_shapes: tuple[Point, ...],
) -> Certificate | None:
    """Shape-bounded exact split cover using conjugate-root divisor roots."""
...
def parallel_direction_conjugate_ideal_cover_witness(
    ...
    """Finite-direction exact split witness using conjugate-root divisor roots."""
...
@cache
def parallel_direction_conjugate_ideal_root_cover_witness(
    target: Point,
    max_root_coordinate: int,
) -> ParallelDirectionConjugateIdealWitness | None:
    """Root-bounded exact split witness using conjugate-root divisor roots."""

    if max_root_coordinate < 2:
        raise ValueError("max_root_coordinate must be at least 2")

    for u, v, _hypotenuse, _root, _unit in primitive_pythagorean_root_directions(
        max_root_coordinate
    ):
        witness = parallel_direction_conjugate_ideal_witness(target, (u, v))
        if witness is not None:
...
@cache
def parallel_direction_conjugate_ideal_root_shape_cover_witness(
    target: Point,
    root_shapes: tuple[Point, ...],
) -> ParallelDirectionConjugateIdealWitness | None:
    """Shape-bounded exact split witness using conjugate-root divisor roots."""
...
def parallel_direction_factor_congruence_data(
    target: Point,
    direction: Point,
    factor: int,
) -> tuple[int, int, int] | None:
    """Return determinant, second-length, and coefficient congruence quotients."""

    if _fast is not None and _all_signed_i64((*target, *direction, factor)):
        try:
            data = _fast.parallel_direction_factor_congruence_data(
                target,
                direction,
                factor,
            )
    ...
    determinant_square = determinant_leg * determinant_leg
    factor_square = factor * factor
    first_denominator = 2 * c * factor
    first_numerator = determinant_square + factor_square
    if first_numerator % first_denominator != 0:
        return None

    second_denominator = 2 * factor * c * c
    second_numerator = determinant_square - factor_square + 2 * factor * dot_product
...
def parallel_direction_factor_pair_row_from_congruence_data(
    target: Point,
    direction: Point,
    factor: int,
) -> ParallelFactorPairRowData | None:
    """Recover factor-pair row data from fixed-direction congruence quotients."""

    if (
        _fast is not None
        and hasattr(_fast, "parallel_direction_factor_pair_row_from_congruence_data")
        and _all_signed_i64((*target, *direction, factor))
    ):
        try:
            data = _fast.parallel_direction_factor_pair_row_from_congruence_data(
                target,
                direction,
                factor,
            )
    ...
    congruence_data = parallel_direction_factor_congruence_data(
        target,
        direction,
        factor,
    )
    if congruence_data is None:
        return None

    determinant_leg, second_length, first_coefficient = congruence_data
    determinant_square = determinant_leg * determinant_leg
    if determinant_square % factor != 0:
        return None
    ...
    u, v = direction
    c = isqrt(u * u + v * v)
    scaled_hypotenuse = c * second_length
    target_x, target_y = target
    dot_product = target_x * u + target_y * v
    if factor + paired_factor != 2 * scaled_hypotenuse:
        raise AssertionError("congruence data did not recover the factor-pair sum")
    if paired_factor - factor + 2 * dot_product != 2 * first_coefficient * c * c:
        raise AssertionError("congruence data did not recover the coefficient row")
    if (paired_factor - factor) % 2 != 0:
        raise AssertionError("factor-pair difference must be even")

    return (
        determinant_leg,
        paired_factor,
        (paired_factor - factor) // 2,
        scaled_hypotenuse,
        second_length,
        first_coefficient,
    )
...
def parallel_direction_primitive_factor_determinant_residue_rows(
    ...
    if _fast is not None and _all_signed_i64((*direction, factor)):
        return tuple(
            tuple(row)
            for row in _fast.parallel_direction_primitive_factor_determinant_residue_rows(
                direction,
                factor,
            )
        )
...
def pythagorean_lattice_pair_strip_linear_congruence(
    ...
    if _fast is not None and _all_signed_i64(
        (*strip_direction, strip_modulus, strip_residue, *first_direction, *second_direction)
    ):
        return tuple(
            _fast.pythagorean_lattice_pair_strip_linear_congruence(
                strip_direction,
                strip_modulus,
                strip_residue,
                first_direction,
...
def two_variable_linear_congruence_gcd(
    first_step: int,
    second_step: int,
    residue: int,
    modulus: int,
) -> tuple[int, bool]:
    """Return gcd(gcd(first_step, second_step), modulus) and solvability."""

    if modulus <= 0:
        raise ValueError("modulus must be positive")
    if _fast is not None and _all_signed_i64(
        (first_step, second_step, residue, modulus)
    ):
        return tuple(
            _fast.two_variable_linear_congruence_gcd(
                first_step,
                second_step,
                residue,
                modulus,
    ...
    linear_gcd = gcd(gcd(first_step, second_step), modulus)
    return (linear_gcd, residue % linear_gcd == 0)


def _crt_residue_mod_lcm(
    first_residue: int,
    first_modulus: int,
    second_residue: int,
    second_modulus: int,
    ...
    shared_modulus = gcd(first_modulus, second_modulus)
    if (first_residue - second_residue) % shared_modulus != 0:
        return None

    first_coprime_part = first_modulus // shared_modulus
    second_coprime_part = second_modulus // shared_modulus
    if second_coprime_part == 1:
        lift = 0
    else:
        lift = (
            ((second_residue - first_residue) // shared_modulus)
            * pow(first_coprime_part % second_coprime_part, -1, second_coprime_part)
        ) % second_coprime_part
    return (first_residue + first_modulus * lift) % lcm(first_modulus, second_modulus)


def two_variable_linear_congruence_pair_data(
    first_step: int,
    second_step: int,
    first_residue: int,
    first_modulus: int,
    ...
    """Return CRT/gcd data for solving two linear congruence rows."""

    if first_modulus <= 0 or second_modulus <= 0:
    ...
    if _fast is not None and _all_signed_i64(
        ...
        return tuple(
            _fast.two_variable_linear_congruence_pair_data(
                first_step,
                second_step,
                first_residue,
                first_modulus,
    ...
    shared_modulus = gcd(first_modulus, second_modulus)
    lcm_modulus = lcm(first_modulus, second_modulus)
    linear_gcd = gcd(gcd(first_step, second_step), lcm_modulus)
    combined_residue = _crt_residue_mod_lcm(
        first_residue,
        first_modulus,
        second_residue,
        second_modulus,
    ...
    if combined_residue is None:
        return (shared_modulus, lcm_modulus, linear_gcd, None, False, False)
    return (
        shared_modulus,
        lcm_modulus,
        linear_gcd,
...
def pythagorean_lattice_pair_strip_crt_data(
    strip_direction: Point,
    first_modulus: int,
    first_residue: int,
    second_modulus: int,
    ...
    first_direction: Point,
    second_direction: Point,
) -> LatticePairStripCrtData:
    """CRT/gcd data for two same-direction determinant strips on one lattice."""
    ...
    if _fast is not None and _all_signed_i64(
        ...
        return tuple(
            _fast.pythagorean_lattice_pair_strip_crt_data(
                strip_direction,
                first_modulus,
                first_residue,
                second_modulus,
    ...
    if not edge_delta(*strip_direction):
        raise ValueError("strip direction must be a legal Pythagorean edge vector")
    if first_modulus <= 1 or second_modulus <= 1:
        raise ValueError("strip moduli must be greater than 1")
    if not edge_delta(*first_direction) or not edge_delta(*second_direction):
        raise ValueError("lattice directions must be legal Pythagorean edge vectors")
    if determinant(first_direction, second_direction) == 0:
        raise ValueError("lattice directions must be independent")

    first_step = determinant(strip_direction, first_direction)
    second_step = determinant(strip_direction, second_direction)
    (
        shared_modulus,
        lcm_modulus,
        linear_gcd,
...
def pythagorean_lattice_pair_same_strip_combined_linear_congruence(
    strip_direction: Point,
    first_modulus: int,
    first_residue: int,
    second_modulus: int,
    ...
    first_direction: Point,
    second_direction: Point,
) -> LatticePairStripLinearCongruence | None:
    """Compress two same-direction strip rows to one lcm coefficient row."""
    ...
    if _fast is not None and _all_signed_i64(
        ...
        row = _fast.pythagorean_lattice_pair_same_strip_combined_linear_congruence(
            strip_direction,
            first_modulus,
            first_residue,
            second_modulus,
    ...
    if not residues_compatible:
        return None
    if combined_residue is None:
        raise AssertionError("compatible CRT row has no combined residue")
    return (
        first_step % lcm_modulus,
        second_step % lcm_modulus,
        combined_residue,
        lcm_modulus,
        linear_gcd,
    )
...
def pythagorean_lattice_pair_same_strip_intersection_residue_count(
    strip_direction: Point,
    first_modulus: int,
    first_residue: int,
    second_modulus: int,
    ...
    """Count residue classes meeting two same-direction strip rows."""
    ...
    if _fast is not None and _all_signed_i64(
        ...
        return tuple(
            _fast.pythagorean_lattice_pair_same_strip_intersection_residue_count(
                strip_direction,
                first_modulus,
                first_residue,
                second_modulus,
    ...
    pair_determinant = abs(determinant(first_direction, second_direction))
    target_modulus = lcm(pair_determinant, row_modulus)
    if not residues_compatible or not is_solvable:
        return (target_modulus, 0)
    residue_count = (
        target_modulus
        * target_modulus
        * linear_gcd
        // (row_modulus * pair_determinant)
    )
    return (target_modulus, residue_count)
...
def pythagorean_lattice_pair_strip_intersection_residue_count(
    ...
    checked_linear_gcd, is_solvable = two_variable_linear_congruence_gcd(
        first_step,
        second_step,
        residue,
        modulus,
    )
    if checked_linear_gcd != linear_gcd:
        raise AssertionError("linear gcd row mismatch")
    if not is_solvable:
...
def pythagorean_layered_conjugate_ideal_certificate(
    ...
    certificate = parallel_direction_conjugate_ideal_root_cover_certificate(
        target,
...
def parallel_direction_conjugate_ideal_root_cover_census(
    max_coordinate: int,
    max_root_coordinate: int,
) -> ParallelDirectionConjugateIdealRootCoverCensus:
    """Census of root-bounded exact split witnesses after structural layers."""
    ...
    uncovered: list[Point] = []
    root_counts: dict[Point, int] = {}
    root_shape_counts: dict[Point, int] = {}
    squareclass_counts: dict[int, int] = {}
    direction_counts: dict[Point, int] = {}
    ...
    for g in range(1, max_coordinate + 1):
        for h in range(1, max_coordinate + 1):
            ...
            root_counts[witness.root] = root_counts.get(witness.root, 0) + 1
            root_shape = gaussian_root_shape(witness.root)
            root_shape_counts[root_shape] = root_shape_counts.get(root_shape, 0) + 1
            squareclass_counts[witness.squareclass] = (
                squareclass_counts.get(witness.squareclass, 0) + 1
            )
            direction_counts[witness.direction] = (
                direction_counts.get(witness.direction, 0) + 1
            )

    return ParallelDirectionConjugateIdealRootCoverCensus(
        max_coordinate=max_coordinate,
        max_root_coordinate=max_root_coordinate,
        target_count=target_count,
        structural_miss_count=structural_miss_count,
        uncovered_targets=tuple(uncovered),
        root_counts=_sorted_count_items(root_counts),
        root_shape_counts=_sorted_count_items(root_shape_counts),
...
def parallel_direction_conjugate_ideal_root_shape_cover_census(
    max_coordinate: int,
    root_shapes: tuple[Point, ...],
) -> ParallelDirectionConjugateIdealRootShapeCoverCensus:
    """Census of exact split witnesses from an explicit root-shape family."""
    ...
    return ParallelDirectionConjugateIdealRootShapeCoverCensus(
        max_coordinate=max_coordinate,
...
def two_one_ray_multiple_of_three_theorem3_certificate(
    multiplier: int,
) -> Certificate | None:
    """Theorem 3 divisor certificate for ``3``-multiples on the ``(2, 1)`` ray."""

    if multiplier <= 0 or multiplier % 3 != 0:
        return None

    parameter = multiplier // 3
    certificate = Certificate(
        target=(2 * multiplier, multiplier),
        midpoint=(12 * parameter, -5 * parameter),
    )
    if not certificate.valid():
        raise AssertionError("multiple-of-three ray certificate formula is invalid")
    return certificate


def two_one_ray_multiple_of_three_theorem3_orbit_certificate(
    target: Point,
) -> Certificate | None:
    """Symmetric certificate for ``3``-multiples of the ``(2, 1)`` ray."""
    ...
    if abs_g == 2 * abs_h:
        base = two_one_ray_multiple_of_three_theorem3_certificate(abs_h)
        if base is None:
            return None
        midpoint_x, midpoint_y = base.midpoint
        return Certificate(
    ...
    if abs_h == 2 * abs_g:
        base = two_one_ray_multiple_of_three_theorem3_certificate(abs_g)
        if base is None:
            return None
        midpoint_x, midpoint_y = base.midpoint
        return Certificate(
...
def two_one_ray_mod60_theorem3_skeleton_certificate(multiplier: int) -> Certificate | None:
    """Mod-60 refinement of the ``(2, 1)`` ray skeleton using Theorem 3."""
...
def two_one_ray_mod60_theorem3_skeleton_residues() -> tuple[int, ...]:
    """Multiplier residues modulo 60 covered after adding the Theorem 3 row."""

    residues: list[int] = []
    for residue in range(60):
        multiplier = residue if residue != 0 else 60
        if two_one_ray_mod60_theorem3_skeleton_certificate(multiplier) is not None:
            residues.append(residue)
    return tuple(residues)


def two_one_ray_mod60_theorem3_skeleton_orbit_certificate(
    target: Point,
) -> Certificate | None:
    """Symmetric certificate for the mod-60 Theorem 3 skeleton on the ray."""
    ...
    if abs_g == 2 * abs_h:
        base = two_one_ray_mod60_theorem3_skeleton_certificate(abs_h)
        if base is None:
            return None
        midpoint_x, midpoint_y = base.midpoint
        return Certificate(
    ...
    if abs_h == 2 * abs_g:
        base = two_one_ray_mod60_theorem3_skeleton_certificate(abs_g)
        if base is None:
            return None
        midpoint_x, midpoint_y = base.midpoint
        return Certificate(
...
def midpoint_axis_certificate(n: int) -> Certificate | None:
    ...
    if _fast is not None and -(1 << 63) <= n < (1 << 63):
        midpoint = _fast.even_axis_certificate_midpoint(n)
        if midpoint is None:
            return None
        return Certificate(target=(n, 0), midpoint=tuple(midpoint))
...
def theorem3_certificate(
    ...
    triple_row = (triple.leg_a, triple.leg_b, triple.hypotenuse)
    if _fast is not None and all(
        -(1 << 63) <= coordinate < (1 << 63)
        for coordinate in (*target, *triple_row, x_sign, y_sign)
    ):
        try:
            midpoint = _fast.theorem3_certificate_midpoint(
                target,
                triple_row,
                x_sign,
                y_sign,
...
def theorem3_divisor_certificate(
    ...
    if _fast is not None and all(
        -(1 << 63) <= coordinate < (1 << 63)
        for coordinate in (*target, *triple_row, x_sign, y_sign, divisor)
    ):
        try:
            midpoint = _fast.theorem3_divisor_certificate_midpoint(
                target,
                triple_row,
                x_sign,
                y_sign,
...
def one_three_ray_theorem3_certificate(multiplier: int) -> Certificate | None:
    """Full ``(1, 3)`` ray certificate from the signed Theorem 3 divisor row."""

    if multiplier <= 0:
        return None

    certificate = Certificate(
        target=(multiplier, 3 * multiplier),
        midpoint=(9 * multiplier, -12 * multiplier),
    )
    if not certificate.valid():
        raise AssertionError("one-three ray Theorem 3 formula is invalid")
    return certificate


def one_three_ray_theorem3_orbit_certificate(target: Point) -> Certificate | None:
    """Symmetric certificate for sign/swap images of the full ``(1, 3)`` ray."""
    ...
    if abs_h == 3 * abs_g:
        base = one_three_ray_theorem3_certificate(abs_g)
        if base is None:
            return None
        midpoint_x, midpoint_y = base.midpoint
        return Certificate(
    ...
    if abs_g == 3 * abs_h:
        base = one_three_ray_theorem3_certificate(abs_h)
        if base is None:
            return None
        midpoint_x, midpoint_y = base.midpoint
        return Certificate(
...
def three_four_five_unit_divisor_ray_data(
    family: str,
    parameter: int,
) -> tuple[Point, int, int] | None:
    """Return ``(ray, x_sign, y_sign)`` for a signed ``3-4-5`` unit-divisor fan."""

    if family == "odd_slope":
        if parameter < 1:
            return None
        return ((parameter, 2 * parameter + 1), 1, -1)
    if family == "steep_odd_slope":
        if parameter < 1:
            return None
        return ((parameter, 8 * parameter + 1), -1, -1)
    if family == "wide_odd_slope":
        if parameter < 0:
            return None
        return ((9 * parameter + 4, 2 * parameter + 1), 1, 1)
    if family == "near_diagonal":
        if parameter < 0:
            return None
        return ((9 * parameter + 1, 8 * parameter + 1), -1, 1)
    raise ValueError("unknown signed 3-4-5 unit-divisor family")


def three_four_five_unit_divisor_ray_certificate(
    family: str,
    parameter: int,
    multiplier: int,
) -> Certificate | None:
    """Certificate for the signed ``3-4-5`` unit-divisor ray-fan table."""
    ...
    certificate = Certificate(
        target=(ray_x * multiplier, ray_y * multiplier),
        midpoint=(x_sign * 3 * product, y_sign * 4 * product),
    )
    if not certificate.valid():
        raise AssertionError("3-4-5 unit-divisor ray formula is invalid")
    return certificate


def _three_four_five_unit_divisor_parameters_for_base(
    x_coordinate: int,
    y_coordinate: int,
    ...
    odd_multiplier = y_coordinate - 2 * x_coordinate
    if odd_multiplier > 0 and x_coordinate % odd_multiplier == 0:
        return ("odd_slope", x_coordinate // odd_multiplier, odd_multiplier)

    steep_multiplier = y_coordinate - 8 * x_coordinate
    if steep_multiplier > 0 and x_coordinate % steep_multiplier == 0:
        return ("steep_odd_slope", x_coordinate // steep_multiplier, steep_multiplier)

    wide_multiplier = 9 * y_coordinate - 2 * x_coordinate
    if wide_multiplier > 0 and y_coordinate % wide_multiplier == 0:
        quotient = y_coordinate // wide_multiplier
        if quotient % 2 == 1:
            parameter = (quotient - 1) // 2
            if (
                parameter >= 0
                and x_coordinate == (9 * parameter + 4) * wide_multiplier
            ):
                return ("wide_odd_slope", parameter, wide_multiplier)

    diagonal_multiplier = 9 * y_coordinate - 8 * x_coordinate
    if diagonal_multiplier > 0 and y_coordinate % diagonal_multiplier == 0:
        quotient = y_coordinate // diagonal_multiplier
        if quotient % 8 == 1:
            parameter = (quotient - 1) // 8
            if (
                parameter >= 0
                and x_coordinate == (9 * parameter + 1) * diagonal_multiplier
            ):
                return ("near_diagonal", parameter, diagonal_multiplier)

    return None


def three_four_five_unit_divisor_ray_orbit_certificate(
    target: Point,
) -> Certificate | None:
    """Symmetric certificate for the signed ``3-4-5`` unit-divisor ray-fan table."""
    ...
    for swap in (False, True):
        base_x, base_y = (abs_h, abs_g) if swap else (abs_g, abs_h)
        parameters = _three_four_five_unit_divisor_parameters_for_base(base_x, base_y)
        if parameters is None:
            continue
        family, parameter, multiplier = parameters
        base = three_four_five_unit_divisor_ray_certificate(
            family,
            parameter,
            multiplier,
        )
...
def three_four_five_odd_slope_ray_certificate(
    ray_parameter: int,
    multiplier: int,
) -> Certificate | None:
    """Full ``(p, 2p+1)`` ray family from the signed ``3-4-5`` row.

    For ``p >= 1`` and ``n >= 1``, the ray divisor of ``(p, 2p+1)`` with
    triple ``(3,4,5)`` and signs ``(1,-1)`` is ``1``.  The divisor-strengthened
    Theorem 3 row therefore certifies every target ``n(p, 2p+1)``.
    """

    if ray_parameter <= 0 or multiplier <= 0:
        return None

    return three_four_five_unit_divisor_ray_certificate(
        "odd_slope",
        ray_parameter,
        multiplier,
    )
...
def _three_four_five_odd_slope_parameters(
    short_coordinate: int,
    long_coordinate: int,
) -> tuple[int, int] | None:
    """Recover ``(p, n)`` from positive coordinates ``(pn, (2p+1)n)``."""

    if short_coordinate <= 0 or long_coordinate <= 0:
        return None
    multiplier = long_coordinate - 2 * short_coordinate
    if multiplier <= 0 or short_coordinate % multiplier != 0:
        return None
    ray_parameter = short_coordinate // multiplier
    if ray_parameter <= 0:
        return None
    return (ray_parameter, multiplier)


def three_four_five_odd_slope_ray_orbit_certificate(
    target: Point,
) -> Certificate | None:
    """Symmetric certificate for the full orbit of the ``(p, 2p+1)`` family."""
    ...
    if abs_h > 2 * abs_g:
        parameters = _three_four_five_odd_slope_parameters(abs_g, abs_h)
        if parameters is None:
            return None
        ray_parameter, multiplier = parameters
        base = three_four_five_odd_slope_ray_certificate(ray_parameter, multiplier)
        if base is None:
            return None
        midpoint_x, midpoint_y = base.midpoint
        return Certificate(
    ...
    if abs_g > 2 * abs_h:
        parameters = _three_four_five_odd_slope_parameters(abs_h, abs_g)
        if parameters is None:
            return None
        ray_parameter, multiplier = parameters
        base = three_four_five_odd_slope_ray_certificate(ray_parameter, multiplier)
...
def five_twelve_thirteen_unit_divisor_ray_data(
    family: str,
    parameter: int,
) -> tuple[Point, int, int] | None:
    """Return ``(ray, x_sign, y_sign)`` for a signed ``5-12-13`` unit fan."""

    if family == "eight_slope":
        if parameter < 1:
            return None
        return ((parameter, 8 * parameter + 1), 1, -1)
    if family == "eighteen_slope":
        if parameter < 1:
            return None
        return ((parameter, 18 * parameter + 1), -1, -1)
    if family == "twentyfive_eight":
        if parameter < 0:
            return None
        return ((25 * parameter + 3, 8 * parameter + 1), 1, 1)
    if family == "twentyfive_eighteen":
        if parameter < 0:
            return None
        return ((25 * parameter + 18, 18 * parameter + 13), -1, 1)
    raise ValueError("unknown signed 5-12-13 unit-divisor family")


def five_twelve_thirteen_unit_divisor_ray_certificate(
    family: str,
    parameter: int,
    multiplier: int,
) -> Certificate | None:
    """Certificate for the signed ``5-12-13`` unit-divisor ray-fan table."""
    ...
    certificate = Certificate(
        target=(ray_x * multiplier, ray_y * multiplier),
        midpoint=(x_sign * 5 * product, y_sign * 12 * product),
    )
    if not certificate.valid():
        raise AssertionError("5-12-13 unit-divisor ray formula is invalid")
    return certificate


def _five_twelve_thirteen_unit_divisor_parameters_for_base(
    x_coordinate: int,
    y_coordinate: int,
) -> tuple[str, int, int] | None:
    """Recover ``(family, parameter, multiplier)`` from positive base coordinates."""
    ...
    eight_multiplier = y_coordinate - 8 * x_coordinate
    if eight_multiplier > 0 and x_coordinate % eight_multiplier == 0:
        return ("eight_slope", x_coordinate // eight_multiplier, eight_multiplier)

    eighteen_multiplier = y_coordinate - 18 * x_coordinate
    if eighteen_multiplier > 0 and x_coordinate % eighteen_multiplier == 0:
        return (
            "eighteen_slope",
            x_coordinate // eighteen_multiplier,
            eighteen_multiplier,
        )

    twentyfive_eight_multiplier = 25 * y_coordinate - 8 * x_coordinate
    if (
        twentyfive_eight_multiplier > 0
        and y_coordinate % twentyfive_eight_multiplier == 0
    ):
        quotient = y_coordinate // twentyfive_eight_multiplier
        if quotient % 8 == 1:
            parameter = (quotient - 1) // 8
            if (
                parameter >= 0
                and x_coordinate
                == (25 * parameter + 3) * twentyfive_eight_multiplier
            ):
                return (
                    "twentyfive_eight",
                    parameter,
                    twentyfive_eight_multiplier,
                )

    twentyfive_eighteen_multiplier = 25 * y_coordinate - 18 * x_coordinate
    if (
        twentyfive_eighteen_multiplier > 0
        and y_coordinate % twentyfive_eighteen_multiplier == 0
    ):
        quotient = y_coordinate // twentyfive_eighteen_multiplier
        if quotient % 18 == 13:
            parameter = (quotient - 13) // 18
            if (
                parameter >= 0
                and x_coordinate
                == (25 * parameter + 18) * twentyfive_eighteen_multiplier
            ):
                return (
                    "twentyfive_eighteen",
                    parameter,
                    twentyfive_eighteen_multiplier,
                )

    return None


def five_twelve_thirteen_unit_divisor_ray_orbit_certificate(
    target: Point,
) -> Certificate | None:
    """Symmetric certificate for the signed ``5-12-13`` unit-divisor table."""
    ...
    for swap in (False, True):
        base_x, base_y = (abs_h, abs_g) if swap else (abs_g, abs_h)
        parameters = _five_twelve_thirteen_unit_divisor_parameters_for_base(
            base_x,
            base_y,
        )
        if parameters is None:
...
def consecutive_euclid_unit_divisor_ray_certificate(
    euclid_parameter: int,
    ray_parameter: int,
    multiplier: int,
) -> Certificate | None:
    """Signed Theorem 3 unit-divisor ray from consecutive Euclid triples.

    For ``r >= 1`` the triple

        ``(2r + 1, 2r(r + 1), 2r^2 + 2r + 1)``

    has ``c - b = 1``.  With signs ``(1,-1)``, the ray
    ``(p, 2r^2 p + 1)`` therefore has fixed divisor ``1`` and every positive
    multiplier is certified by the divisor-strengthened Theorem 3 row.
    """

    if euclid_parameter < 1 or ray_parameter < 1 or multiplier <= 0:
        return None
    ...
    certificate = Certificate(
        target=(ray[0] * multiplier, ray[1] * multiplier),
        midpoint=(leg_a * product, -leg_b * product),
    )
    if not certificate.valid():
        raise AssertionError("consecutive Euclid unit-divisor formula is invalid")
    return certificate


def _consecutive_euclid_unit_divisor_parameters_for_base(
    x_coordinate: int,
    y_coordinate: int,
) -> tuple[int, int, int] | None:
    """Recover ``(r, p, multiplier)`` from ``(pm, (2r^2p+1)m)``."""
    ...
    while True:
        slope_factor = 2 * r * r
        multiplier = y_coordinate - slope_factor * x_coordinate
        if multiplier <= 0:
            return None
        if x_coordinate % multiplier == 0:
            ray_parameter = x_coordinate // multiplier
            if ray_parameter >= 1:
                return (r, ray_parameter, multiplier)
        r += 1


def consecutive_euclid_unit_divisor_ray_orbit_certificate(
    target: Point,
) -> Certificate | None:
    """Symmetric certificate for the consecutive-Euclid unit-divisor fan."""
    ...
    for swap in (False, True):
        base_x, base_y = (abs_h, abs_g) if swap else (abs_g, abs_h)
        parameters = _consecutive_euclid_unit_divisor_parameters_for_base(
            base_x,
            base_y,
        )
        if parameters is None:
            continue
        euclid_parameter, ray_parameter, multiplier = parameters
        base = consecutive_euclid_unit_divisor_ray_certificate(
            euclid_parameter,
            ray_parameter,
            multiplier,
        )
...
def consecutive_euclid_affine_strip_certificate(
    euclid_parameter: int,
    free_coordinate: int,
) -> Certificate | None:
    """Swapped-leg unit-divisor strip from consecutive Euclid triples.

    For ``r >= 1`` and ``h >= 1``, the swapped consecutive Euclid triple

        ``(2r(r + 1), 2r + 1, 2r^2 + 2r + 1)``

    has ``c - a = 1`` and ``c - b = 2r^2``.  The signed Theorem 3 row with
    signs ``(1,-1)`` certifies the affine strip target ``(2r^2 h - 1, h)``.
    """

    if euclid_parameter < 1 or free_coordinate < 1:
        return None
    ...
    certificate = Certificate(
        target=(g, h),
        midpoint=(leg_a * coefficient, -leg_b * coefficient),
    )
    if not certificate.valid():
        raise AssertionError("consecutive Euclid affine strip formula is invalid")
    return certificate


def consecutive_euclid_affine_strip_orbit_certificate(
    target: Point,
    euclid_parameter: int,
) -> Certificate | None:
    """Symmetric certificate for the swapped-leg consecutive-Euclid strip."""

    if euclid_parameter < 1:
        raise ValueError("euclid_parameter must be positive")
    ...
    for swap in (False, True):
        for x_sign in (-1, 1):
            for y_sign in (-1, 1):
                candidate_target = signed_swap_point(target, x_sign, y_sign, swap)
                g, h = candidate_target
                if h < 1 or g != 2 * r * r * h - 1:
                    continue
                base = consecutive_euclid_affine_strip_certificate(r, h)
                if base is None:
                    continue
                certificate = sign_swap_certificate(base, target)
                if certificate is not None:
...
def consecutive_parameter_odd_axis_certificate(n: int) -> SharedLegAxisCertificate | None:
    ...
    if _fast is not None and -(1 << 63) <= n < (1 << 63):
        try:
            data = _fast.consecutive_odd_axis_certificate_data(n)
        except OverflowError:
            data = None
        else:
            if data is None:
